...
<x-slot:breadcrumb>
    <x-slot:header>Edit</x-slot>
    <x-slot:icon_header>
        <i class="ph-duotone  ph-plus f-s-16"></i> Edit
    </x-slot>
    <x-slot:sub_header>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @endsession
                    <form wire:submit="update" class="row g-3 app-form">
                        <div class="col-md-12">
                        ...
                        <div class="col-md-12">
                            ...
                            <select class="form-select @error('form.type') is-invalid @enderror" id="resource-type" aria-label="select type" wire:model="form.type">
                                ...
                                @foreach(\App\ResourceTypeEnum::cases() as $type)
                                    <option value="{{ $type->value }}" {{ $type === $resource?->type ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                            @error('form.type')
                            <div class="mt-1">
